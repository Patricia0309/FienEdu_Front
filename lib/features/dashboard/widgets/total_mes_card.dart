...
class TotalMesCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    print("DEBUG WIDGET: TotalMesCard CONSTRUIDO con gastos: $gastos, presupuesto: $presupuesto, extra: $gastoExtra");
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow(icon: Icons.arrow_downward, color: Colors.orange.shade700, label: 'Gastado (dentro)', amount: gastosDentroDelPresupuesto.toStringAsFixed(0)),

          if (gastoExtra > 0) ...[
            const SizedBox(height: 12),
            _buildSummaryRow(icon: Icons.warning_amber_rounded, color: Colors.red.shade600, label: 'Gasto Extra', amount: gastoExtra.toStringAsFixed(0)),
          ],

          const Divider(height: 32),
          _buildSummaryRow(label: 'Restante', amount: restante.toStringAsFixed(0), isBalance: true),
          const SizedBox(height: 24),
          ...
          if (showChart) ...[
                ...
                children: [
                   ...
                   PieChart(
                      PieChartData(
                         ...
                         sections: [
                           PieChartSectionData(
                             value: chartValue, // El valor visual se basa en el porcentaje
                             color: AppColors.element,
                             radius: 15, showTitle: false,
                           ),
                           PieChartSectionData(
             ...
             Row(
               ...
               children: [
                 ...
                 _buildLegendItem(AppColors.element.withOpacity(0.2), 'Restante'),
               ],
             )
          ] else ...[
             Center(
  ...
  Widget _buildSummaryRow({IconData? icon, Color? color, required String label, required String amount, bool isBalance = false}) {
     final amountStyle = AppTextStyles.body.copyWith(fontWeight: FontWeight.bold, color: color ?? AppColors.primary);
     return Row(
